...
This could result in a false positive if multiple files have the same name, so
it will be delivered as an opt-in feature.

For example if you have a file `/usr/bin/foo` and `/bin/foo` and you only
specify `foo` in the allowlist file, one of the files will fail withn the
verifier (assuming they have different digests). Typically though, considering
$PATH is available and dictating file namespace execution, this is very unlikely.
